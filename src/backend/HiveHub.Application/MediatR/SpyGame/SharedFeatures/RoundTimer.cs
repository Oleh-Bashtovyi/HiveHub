...
public static class RoundTimer
{
    public static void CheckAndResolveVotersForRoundTimerStop(SpyRoom room, SpyGameEventsContext context, ILogger logger)
    {
        if (room.GameState.RoundTimerState.IsStopped || !room.IsInGame())
        {
            return;
        ...
        if (votesCount >= requiredVotes)
        {
            RoundEnd.BeginFinalVoting(room, context, logger);
        }
    }
